 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tool_switchenrol_tracker {
     ...
     */
    protected $columns = array('line', 'result', 'shortname', 'enrolold', 'enrolnew', 'status');
    ...
    public function results($total, $updated, $errors) {
        ...
        $message = array(
            get_string('coursestotal', 'tool_switchenrol', $total),
            get_string('coursesupdated', 'tool_switchenrol', $updated),
            get_string('courseserrors', 'tool_switchenrol', $errors)
        );
    ...
    public function output($line, $outcome, $status, $data) {
        ...
        if ($this->outputmode == self::OUTPUT_PLAIN) {
            $message = array(
                ...
                isset($data['enrolold']) ? $data['enrolold'] : '',
                isset($data['enrolnew']) ? $data['enrolnew'] : ''
            );
            ...
            echo html_writer::tag('td', isset($data['enrolold']) ? $data['enrolold'] : '', array('class' => 'c' . $ci++));
            echo html_writer::tag('td', isset($data['enrolnew']) ? $data['enrolnew'] : '', array('class' => 'c' . $ci++));
    ...
    public function start() {
        ...
        if ($this->outputmode == self::OUTPUT_PLAIN) {
            ...
            echo html_writer::start_tag('table', array('class' => 'generaltable boxaligncenter flexible-wrap',
                'summary' => get_string('switchenrolsresult', 'tool_switchenrol')));
            echo html_writer::start_tag('tr', array('class' => 'heading r' . $this->rownb));
            echo html_writer::tag('th', get_string('csvline', 'tool_switchenrol'),
                array('class' => 'c' . $ci++, 'scope' => 'col'));
            echo html_writer::tag('th', get_string('result', 'tool_switchenrol'), array('class' => 'c' . $ci++, 'scope' => 'col'));
            echo html_writer::tag('th', get_string('shortname'), array('class' => 'c' . $ci++, 'scope' => 'col'));
            echo html_writer::tag('th', get_string('enrolold', 'tool_switchenrol'), array('class' => 'c' . $ci++, 'scope' => 'col'));
            echo html_writer::tag('th', get_string('enrolnew', 'tool_switchenrol'), array('class' => 'c' . $ci++, 'scope' => 'col'));
